...
def get_data(args):
    image_dataset = torchvision.datasets.ImageNet(args.imagenet_folder, split="val", loader=Image.open, transform=make_transform(args.image_size, args.normalization_mode))
    ...
    text_collate = Collator(args.tokenizer)
    text_datasets = []
    ...
    if "," not in args.prompts:
        prompt_names = list(args.prompts)
    else:
...
def make_transform(image_size=224, normalization_mode="imagenet"):
...
class Collator:
    def __init__(self, tokenizer, max_sequence_length=77):
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer)
        self.max_sequence_length = max_sequence_length

    def __call__(self, examples):
        text_features = self.tokenizer(examples, truncation=True, padding="max_length", max_length=self.max_sequence_length, return_tensors="pt")
